...
- Fontes: **Cormorant Garamond** (serif editorial) + **Inter** (sans), carregadas via Google Fonts no `index.html`
...
export const contact = {
  ...
  agency: { name: "Wave Labs Performance", url: "https://www.instagram.com/wavelabsperformance/" },
};
```

### Mídias
Vou gerar imagens cinematográficas com `imagegen` para `/public/images/` (hero, condutora, jornada×4, vivências×4, espaço, CTA final). Vídeos serão **arquivos placeholder vazios** com nome correto — você substitui pelos reais sem tocar no código. Documentarei isso num `public/MEDIA.md`.
...
- **VideoPlayer** detecta touch via `pointer: coarse` para alternar entre hover-preview e tap-preview
- **Formulário WhatsApp** monta `https://wa.me/5579996454440?text=...` com dados do form
- Build output: `dist/` (default Vite), Vercel auto-detecta

---

## O que NÃO entra nesta entrega
- Integração com CRM / e-mail marketing (apenas link WhatsApp)
- CMS — conteúdo direto no JSX conforme você pediu
- Vídeos reais (você fornece depois — só os placeholders)
- IDs reais de Meta Pixel / GA (placeholders comentados)

---

Posso seguir com essa migração completa + construção da landing? É um trabalho extenso (≈25-30 arquivos novos, geração de ~10 imagens), então confirmo antes de começar.